...
def Initialize(args):
    ...
    #Check for Args
    if len(args)<2:
        print("USAGE: python anchovy.0.0.py pathto/input.sam query")
        exit()
...
def blockDist(seq_query):
    ...
    contextSeq="|".join([seq[(minPos-quL):minPos],seq[min(len(seq),minPos+quL):min(len(seq),(minPos+quL))]])
...
if __name__=="__main__":
    t0=(time.time())
    query, pdSam, outfileName = Initialize(sys.argv)
