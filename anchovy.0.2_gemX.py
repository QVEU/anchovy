...
def Initialize(args):
    ...
    #Query
    query=args[3].upper()
    #5' query GEM-X: CTACACGACGCTCTTCCGATCTNNNNNNNNNNNNNNNNNNNNNNNNNNTTTCTTATAT #changed  PTD: 5/29/25
    whitelist=args[2]
...
def loadBC(whitelist):
    ...
    with open(whitelist,"r") as IF:
        CBCs=[lin.split("\t")[0].strip() for lin in IF]
    size=len(CBCs)
...
def cellMatch(input): #TUPLE including (readID, readSeq, matchSeq, matchseq, CBCs, blocks):
    ...
    return(CBCs.iloc[minPos][0], minD,    minPos,          matchPos,           offset,              matchblock,              matchseq,             readID,         readSeq,              matchseq[38:49])#changed to 49 to adjust for new UMI length

#Pooled cell ID function
def cellIDPool(pdSam, pdCBCs, nthreads=16):
    #print(len(pdSam))
    blocks=np.array(["CTACACGACGCTCTTCCGATCT"+i+"NNNNNNNNNNNNTTTCTTATAT" for i in pdCBCs.CBC])#12 UMI added 5/29/25 PTD
    pdSam=pdSam[pdSam.minD<41]#filter by distance of index sequences cassette to template - Added 2 to adjust for new UMI 5/29/25 PTD
